...
public class SmallFontRenderer extends FontRenderer
{
    ...
    public SmallFontRenderer(GameSettings par1GameSettings, ResourceLocation par2ResourceLocation, TextureManager par3TextureManager)
    {
    		super(par1GameSettings, par2ResourceLocation, par3TextureManager, true);
    ...
    {
        return par2 == 32 ? 4.0F : (par1 > 0 && !this.unicodeFlag ? this.renderSmallDefaultChar(par1 + 32, par3) : this.renderSmallUnicodeChar(par2, par3));
    }

    /**
     * Render a single character with the default.png font at current (posX,posY) location...
     */
    private float renderSmallDefaultChar (int par1, boolean par2)
    {
     ...
     */
    private float renderSmallUnicodeChar (char par1, boolean par2)
    {
    ...
    private void renderStringAtPos (String par1Str, boolean par2)
    {
        for (int i = 0; i < par1Str.length(); ++i)
        {
            ...
            {
            		String allowedCharacters = "abcdefghijjklmnopqrstuvwxyz";
                ...
                if (this.randomStyle && j > 0)
                {
                    do
                    {
                        k = this.fontRandom.nextInt(allowedCharacters.length());
                ...
                if (this.strikethroughStyle)
                {
                    ...
                    BufferBuilder br = tessellator.getBuffer();
                    
                    GL11.glDisable(GL11.GL_TEXTURE_2D);
                    
                    br.begin(GL11.GL_QUADS, DefaultVertexFormats.POSITION);
                    br.pos((double) this.posX, (double) (this.posY + (float) (this.FONT_HEIGHT / 2)), 0.0D).endVertex();
                    br.pos((double) (this.posX + f1), (double) (this.posY + (float) (this.FONT_HEIGHT / 2)), 0.0D).endVertex();
                    br.pos((double) (this.posX + f1), (double) (this.posY + (float) (this.FONT_HEIGHT / 2) - 1.0F), 0.0D).endVertex();
                    br.pos((double) this.posX, (double) (this.posY + (float) (this.FONT_HEIGHT / 2) - 1.0F), 0.0D).endVertex();
                    tessellator.draw();
                ...
                if (this.underlineStyle)
                {
                    ...
                    br.pos((double) (this.posX + (float) l), (double) (this.posY + (float) this.FONT_HEIGHT), 0.0D).endVertex();
                    br.pos((double) (this.posX + f1), (double) (this.posY + (float) this.FONT_HEIGHT), 0.0D).endVertex();
                    br.pos((double) (this.posX + f1), (double) (this.posY + (float) this.FONT_HEIGHT - 1.0F), 0.0D).endVertex();
                    br.pos((double) (this.posX + (float) l), (double) (this.posY + (float) this.FONT_HEIGHT - 1.0F), 0.0D).endVertex();
                    tessellator.draw();
    ...
    {
        return Arrays.asList(this.wrapSmallFormattedStringToWidth(par1Str, par2).split("\n"));
    }

    /**
     * Inserts newline and formatting into a string to wrap it within the specified width.
     */
    String wrapSmallFormattedStringToWidth (String par1Str, int par2)
    {
        ...
        else
        {
            ...
            String s2 = getFormatFromSmallString(s1) + par1Str.substring(j + (flag ? 1 : 0));
            return s1 + "\n" + this.wrapSmallFormattedStringToWidth(s2, par2);
        }
    }
     ...
     */
    private static String getFormatFromSmallString (String par0Str)
    {
        String s1 = "";
        int i = -1;
